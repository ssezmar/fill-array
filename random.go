...
type GetUniqueIntegresInput struct {
	SizeX int
	SizeY int
	RandomLimit int
}
...
func (i GetUniqueIntegresInput) Validate() error {
	...
	if i.RandomLimit < i.SizeX*i.SizeY {
		return errors.New(
			fmt.Sprintf("can't generate unique with limit %d for %dx%d sized array", 
...
func (r *Randomizer) getUniquerandomIntegers(input GetUniqueIntegresInput) ([]int, error) {
